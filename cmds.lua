...
local function IgniteTarget(Target:Player)
	if Target ~= nil and typeof(Target) == "Instance" and Target:IsA("Player") and Target.Name ~= tostring(Player.Name) and Target.Character and Player.Character then
		local Char = Target.Character
		local Tool = Player.Backpack:FindFirstChild("pistols") or Player.Backpack:FindFirstChild("sniper") or Player.Backpack:FindFirstChild("flamethrower") or Character:FindFirstChildWhichIsA("Tool")
		...
		if Tool ~= nil then
			...
			if ModuleSetting then
				...
				if GunScript_Server then
					...
					local A2_1 = {RequiredModule.MeleeKnockback, RequiredModule.MeleeLifesteal, RequiredModule.MeleeAffliction, RequiredModule.MeleeAfflictionName, RequiredModule.MeleeAfflictChance} -- Original
					local A2_2 = {20, RequiredModule.MeleeLifesteal, true, "IgniteScript", 100}
...
local function ELockTarget(Target:Player)
	if Target ~= nil and typeof(Target) == "Instance" and Target:IsA("Player") and Target.Name ~= tostring(Player.Name) and Target.Character and Player.Character then
		_G.LockedPlayers[tostring(Target.Name)] = true

		while _G.LockedPlayers[tostring(Target.Name)] do
			local Char = Target.Character
			local Tool = Player.Backpack:FindFirstChild("pistols") or Player.Backpack:FindFirstChild("sniper") or Player.Backpack:FindFirstChild("flamethrower") or Character:FindFirstChildWhichIsA("Tool")
...
Player.Chatted:Connect(function(Message)
	if Message ~= nil and typeof(Message) == "string" then
		...
		if SplitMessage[1] == "/e" and SplitMessage[2] ~= nil and SplitMessage[3] ~= nil then
			...
			if Found then
				if string.lower(SplitMessage[2]) == "icify" then
					return IcifyTarget(Target)
				elseif string.lower(SplitMessage[2]) == "ignite" then
					return IgniteTarget(Target)
				elseif string.lower(SplitMessage[2]) == "elock" then
					return ELockTarget(Target)
				elseif string.lower(SplitMessage[2]) == "unlock" then
					if _G.LockedPlayers[tostring(Target.Name)] ~= nil then 
						_G.LockedPlayers[tostring(Target.Name)] = false
					end
				end
